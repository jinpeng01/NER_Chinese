...
# i = U.other_to_int('NNB')
# print(i)
# print(U.text_to_arr('我是'))
# print(U.other_size())
    ...
    with open(fileName, 'r', encoding='utf-8') as file:
        for line in file.readlines():
            ...
            if (len(items) > 2):
                ...
                other_feature_list_item.append(items[1])
                target_list_item.append(items[2])
                # print(items[0]+'0',items[1]+'1',items[2]+'2')
            else:

                word_index_vector_item = U.toIndex(word_list_item, U.word_to_int_table)
                word_index_vector_item = U.padding(word_index_vector_item, U.vocab_size() + 2, max_length)
                if (len(word_index_vector_item) > max_length):
                    print('len(word_index_vector_item)', len(word_index_vector_item))
                word_index_vector.append(word_index_vector_item)
                # print(word_index_vector_item)


                other_feature_index_vector_item = U.toIndex(other_feature_list_item, U.other_to_int_table)
                other_feature_index_vector_item = U.padding(other_feature_index_vector_item, U.other_size() + 2
                                                            , max_length)

                other_feature_index_vector.append(other_feature_index_vector_item)
                # print(other_feature_index_vector_item)


                target_index_vector_item = U.toIndex(target_list_item, U.target_to_int_table)
                target_index_vector_item = U.padding(target_index_vector_item, U.target_voacab_size() + 2,
                                                     max_length)
                ...
                other_feature_list_item = []
                target_list_item = []
                # print(other_feature_index_vector)
    word_list = np.array(word_index_vector)
    # print(word_list.shape)
    other_feature_list = np.array(other_feature_index_vector)
    # print(other_feature_list.shape)
    target_list = np.array(target_index_vector)
    print(target_list.shape[1])
    startindex = 0
    for X,M in zip(word_list,other_feature_list):
        X = np.reshape(X,[1,-1])
        M = np.reshape(M,[1,-1])
